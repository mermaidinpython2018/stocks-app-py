...
# TODO (HINT: use the daily_prices variable, and don't worry about the CSV file anymore :-)
    ...
    if float(daily_prices[0]["close"]) < avg_low_price * 0.2:
        print("Buy!" + " " + "Due to the stock's latest closing price is less than 20% above its recent average low!")
    else:
        print("Don't Buy!" + " " + "Due to the stock's latest closing price is more than 20% above its recent average low!")
...
if __name__ == '__main__': #only execute if file invoked from the command-line, not when imported into other files, like tests
    ...
    #while True:
    if symbol.isnumeric() == True or len(symbol)!=4:
        print("Wrong message, please check your symbol!")
        sys.exit()
